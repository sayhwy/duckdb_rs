...
impl IndexConstraintType {
    pub fn is_unique(&self) -> bool { matches!(self, Self::Unique | Self::Primary) }
...
impl Value {
    ...
    pub fn as_str(&self) -> Option<&str> {
        if let Value::Text(s) = self { Some(s.as_str()) } else { None }
    }
    ...
    pub fn to_sql_string(&self) -> String {
        match self {
            ...
            Value::Blob(b)    => format!("decode('{}')", b.iter().map(|x| format!("{:02x}", x)).collect::<String>()),
        }
    }
    ...
    pub fn serialize(&self) -> Vec<u8> {
        ...
        match self {
            ...
            Value::Boolean(b) => { buf.push(1); buf.push(*b as u8); }
            Value::Integer(n) => { buf.push(2); buf.extend_from_slice(&n.to_le_bytes()); }
            Value::Float(f)   => { buf.push(3); buf.extend_from_slice(&f.to_le_bytes()); }
            Value::Text(s)    => {
                buf.push(4);
...
impl LogicalType {
    pub fn new(id: LogicalTypeId) -> Self {
        Self { id, width: 0, scale: 0, children: Vec::new() }
    }
    ...
    pub fn timestamp() -> Self { Self::new(LogicalTypeId::Timestamp) }
    pub fn blob()      -> Self { Self::new(LogicalTypeId::Blob) }

    pub fn decimal(width: u8, scale: u8) -> Self {
        Self { id: LogicalTypeId::Decimal, width, scale, children: Vec::new() }
    }
    ...
    pub fn deserialize(data: &[u8]) -> Option<(Self, usize)> {
        ...
        let id = match tag {
            ...
            13 => {
                ...
                let w = data[pos]; let s = data[pos + 1]; pos += 2;
            ...
            14 | 15 | 16 => {
                let list_id = match tag { 14 => LogicalTypeId::List, 15 => LogicalTypeId::Struct, _ => LogicalTypeId::Map };
                if data.len() < pos + 4 { return None; }
                ...
                return Some((LogicalType { id: list_id, width: 0, scale: 0, children }, pos));
            }
...
impl ColumnDefinition {
    pub fn new(name: String, logical_type: LogicalType) -> Self {
        Self { name, logical_type, default_value: None, generated_expression: None, comment: None }
    }

    pub fn is_generated(&self) -> bool { self.generated_expression.is_some() }
    ...
    pub fn deserialize(data: &[u8]) -> Option<(Self, usize)> {
        ...
        let tlen = u32::from_le_bytes(data[pos..pos+4].try_into().ok()?) as usize; pos += 4;
        let (logical_type, _) = LogicalType::deserialize(&data[pos..pos+tlen])?; pos += tlen;
        let default_value = if data[pos] == 0 {
            pos += 1; None
        } else {
            pos += 1;
            let dlen = u32::from_le_bytes(data[pos..pos+4].try_into().ok()?) as usize; pos += 4;
            let s = String::from_utf8(data[pos..pos+dlen].to_vec()).ok()?; pos += dlen;
            Some(s)
        };
        let generated_expression = if data[pos] == 0 {
            ...
            pos += 1;
            let glen = u32::from_le_bytes(data[pos..pos+4].try_into().ok()?) as usize; pos += 4;
            let s = String::from_utf8(data[pos..pos+glen].to_vec()).ok()?; pos += glen;
            Some(s)
        };
        Some((Self { name, logical_type, default_value, generated_expression, comment: None }, pos))
    }
}
...
impl ColumnList {
    ...
    pub fn add_column(&mut self, col: ColumnDefinition) { self.columns.push(col); }
    ...
    pub fn deserialize(data: &[u8]) -> Option<(Self, usize)> {
        ...
        for _ in 0..count {
            ...
            let (col, _) = ColumnDefinition::deserialize(&data[pos..pos+clen])?; pos += clen;
            columns.push(col);
        }
...
pub enum ConstraintType {
    ...
    Unique  { columns: Vec<String>, is_primary: bool },
    ForeignKey {
        table: String,
    ...
    pub fn to_sql(&self) -> String {
        match self {
            ConstraintType::NotNull { column } =>
                format!("{} NOT NULL", column),
            ConstraintType::Check { expression } =>
                format!("CHECK ({})", expression),
            ConstraintType::Unique { columns, is_primary } => {
                let kw = if *is_primary { "PRIMARY KEY" } else { "UNIQUE" };
                format!("{} ({})", kw, columns.join(", "))
            }
            ConstraintType::ForeignKey { table, fk_columns, pk_columns } =>
                format!("FOREIGN KEY ({}) REFERENCES {} ({})",
                    fk_columns.join(", "), table, pk_columns.join(", ")),
        }
    }
}
...
impl CreateInfo {
    ...
    pub fn serialize(&self) -> Vec<u8> {
        ...
        match &self.comment {
            None    => buf.push(0),
            Some(v) => { buf.push(1); let vb = v.serialize(); buf.extend_from_slice(&(vb.len() as u32).to_le_bytes()); buf.extend_from_slice(&vb); }
        }
    ...
    pub fn deserialize(data: &[u8]) -> Option<(Self, usize)> {
        let mut pos = 0;
        let catalog_type = CatalogType::from_u8(data[pos]); pos += 1;
        let (catalog, n) = Self::read_string(&data[pos..])?; pos += n;
        let (schema, n)  = Self::read_string(&data[pos..])?; pos += n;
        let temporary = data[pos] != 0; pos += 1;
        let internal  = data[pos] != 0; pos += 1;
        ...
        let comment = if data[pos] == 0 {
            ...
            pos += 1;
            let vlen = u32::from_le_bytes(data[pos..pos+4].try_into().ok()?) as usize; pos += 4;
            let (v, _) = Value::deserialize(&data[pos..pos+vlen])?; pos += vlen;
            Some(v)
        };
        let tag_count = u32::from_le_bytes(data[pos..pos+4].try_into().ok()?) as usize; pos += 4;
        let mut tags = HashMap::new();
        for _ in 0..tag_count {
            let (k, n) = Self::read_string(&data[pos..])?; pos += n;
            let (v, n) = Self::read_string(&data[pos..])?; pos += n;
            tags.insert(k, v);
        }
        Some((Self { catalog_type, catalog, schema, temporary, internal, on_conflict, comment, tags }, pos))
    }
...
impl CreateSchemaInfo {
    ...
    pub fn schema_name(&self) -> &str { &self.base.schema }
}
...
impl CreateTableInfo {
    pub fn new(schema: String, table: String) -> Self {
        ...
        base.schema = schema;
        Self { base, table, columns: ColumnList::new(), constraints: Vec::new() }
    }
    ...
    pub fn to_sql(&self) -> String {
        ...
        format!("CREATE TABLE {}.{} (\n  {}\n)",
            self.base.schema, self.table, parts.join(",\n  "))
    }
}
...
impl CreateViewInfo {
    pub fn new(schema: String, view_name: String, query: String) -> Self {
        ...
        base.schema = schema;
        Self { base, view_name, aliases: Vec::new(), types: Vec::new(), query, column_names: Vec::new() }
    }
}
...
impl CreateFunctionInfo {
    pub fn new(catalog_type: CatalogType, schema: String, name: String) -> Self {
        ...
        base.schema = schema;
        Self { base, name, or_replace: false, descriptions: Vec::new() }
    }
}
...
impl CreateCollationInfo {
    pub fn new(schema: String, name: String) -> Self {
        ...
        base.schema = schema;
        Self { base, name, combinable: false, not_required_for_equality: false }
    }
}
...
pub enum AlterKind {
    ...
    AddColumn       { column: ColumnDefinition, if_not_exists: bool },
    RemoveColumn    { column_name: String, if_exists: bool, cascade: bool },
    AlterColumnType { column_name: String, new_type: LogicalType, cast_expr: Option<String> },
    SetDefault      { column_name: String, default_value: Option<String> },
    SetNotNull      { column_name: String },
    DropNotNull     { column_name: String },
    SetComment      { new_comment: Option<Value> },
    SetColumnComment{ column_name: String, comment: Option<Value> },
    RenameView      { new_name: String },
    AddForeignKey   { constraint: ConstraintType },
    DropConstraint  { constraint_name: String, cascade: bool },
    SetTags         { tags: HashMap<String, String> },
    AlterFunction   { name: String, or_replace: bool },
}
