...
fn main() {
    ...
    for (i, s) in students.iter().enumerate() {
        ...
        // We mirror `checkpoint_manager.rs`'s `string_t` extraction:
        //   [0..4]   u32 length
        //   [4..4+n] bytes
        // with inline<=12 for now.
        let off = i * 16;
...
/// enrollment_day 为自 1970-01-01 起的天数（i32）。
///
/// 2000-01-01 = days 10957 from 1970-01-01
fn generate_students(n: usize) -> Vec<(i32, &'static str, i32, f32, bool, i32)> {
    ...
    for i in 0..n {
        ...
        let gpa = 2.0 + (i as f32 % 20.0) * 0.1; // 2.0 ~ 3.9
        let is_active = i % 3 != 0; // 每 3 个学生有 1 个不活跃
        // enrollment_day: 从 2000-01-01 (天数 10957) 起，每个学生差 30 天
...
fn write_varchar_inline_string_t(dst: &mut [u8], s: &str) {
    ...
    // `checkpoint_manager.rs` currently only supports inline strings with len <= 12.
    assert!(
        bytes.len() <= 12,
    ...
    // Next bytes: string payload (no trailing '\0')
    dst[4..4 + bytes.len()].copy_from_slice(bytes);
    // The remaining bytes stay as 0 (DataChunk initializes them to 0).
    dst[4 + bytes.len()..].fill(0);
}

/// 用 duckdb.exe 读取并打印前 10 行，验证多种查询。
